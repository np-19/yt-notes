...
export async function getVideoDetailsAndTranscript(videoId: string): Promise<VideoInfo> {
  ...
  // Fallback 2: Gemini AI Transcript Generation (when all network scrapers/proxies are blocked)
  if (transcript.length === 0 && title) {
    console.log(`[yt.transcript] Scrapers returned no transcript. Attempting Gemini AI transcript reconstruction for "${title}"...`);
    try {
      const aiTranscript = await executeWithModelFallback<TranscriptEntry[]>("generateTranscriptWithGemini", async (model) => {
...
Provide at least 15 to 30 detailed chronological transcript snippets covering the fundamental concepts, workflows, code/math, and takeaways of this lecture.
CRITICAL: Return ONLY raw JSON (an array [ {...}, {...} ]) with no markdown fences, no formatting, and no commentary.`;
      ...
      console.warn(`[yt.transcript] Gemini transcript generation failed:`, err?.message || err);
    }
  }
